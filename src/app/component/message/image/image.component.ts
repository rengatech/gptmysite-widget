...
export class ImageComponent implements OnInit {
  ...
  onClickImage(){
    ...
    ifrm.setAttribute('id','GPTMysite-image-preview');
    ...
    iframeContent += '<style> .GPTMysite-popup {position: absolute; inset: 1px; outline-offset: -5px; background-color: rgba(0, 0, 0, 0.35); border-radius:16px; will-change: opacity;}'
    iframeContent +=    '.GPTMysite-popup-content { position: fixed; inset: 0px; width: 100%;  height: 100%; display: flex;  justify-content: center; align-items: center; outline: 0px;}'
    iframeContent +=    '.GPTMysite-popup-button { display: flex; align-items: center; justify-content: center; width: 35px; height: 35px; position: absolute; top: 0px; right: 0px; background-color: transparent; border: none; cursor: pointer; margin: 9px; padding: 0px; }'
    iframeContent +=    '.GPTMysite-popup-image { max-height: 80vh; max-width: 80vw; }'
    iframeContent += '</style>'
    iframeContent += '</head>';
    iframeContent += '<body>'
    iframeContent +=  '<div class="frame-root" id="frame-root">'
    iframeContent +=    '<div class="frame-content">'
    iframeContent +=      '<div class="GPTMysite-popup" style="opacity: 1;"></div>'
    iframeContent +=      '<div role="button" tabindex="-1" class="GPTMysite-popup-content">'
    // iframeContent +=         '<button id="button" type="button" data-testid="closeButton" class="GPTMysite-popup-button">'
    ...
    iframeContent +=           '<img src="'+this.metadata.src+'" class="GPTMysite-popup-image" id="image-popup">'
    iframeContent +=         '</a>'
    iframeContent +=      '</div>'
    iframeContent +=   '</div>'
    iframeContent +='</body>'
    ...
    ifrm.onload = function(ev) {
      var iframe = window.document.getElementById('GPTMysite-image-preview')
